...
var width = document.body.clientWidth,
    height = d3.max([document.body.clientHeight-440, 240]);
...
d3.csv("dataset.csv", function(raw_data) {
...
function export_csv() {
  ...
  var rows = _.difference(data, actives()).map(function(row) {
...
d3.select("#refresh-data").on("click", refresh_data);
...
function search(selection,str) {
  ...
  return _(selection).filter(function(d) { return pattern.exec(d.country); });
}

//listen image click event
country_unit=d3.select("ul#contry_selector").selectAll(".country_unit")
  			   .data(g20members).enter()
  			   .append("li")
  			   .attr("class","country_unit");
  			   
block_unit=country_unit.append("a")
					   .attr("class","block_unit")
					   ...
					   .on("click",function(d){
					   	if(country_name==d){
                        	...
                        	country_name=d;
                        	data=temp;
                        new_data=data.filter(function(d){ return d.country==country_name});
                        data = new_data;
                        rescale();  
					   	} });
...
_span=block_unit.append("span")
				.attr("class","image_span");
				
_span.append("img")
	 .attr("src",function(d){ return "image/"+d+".png"});
	 
block_unit.append("p")
          .text(function(d){
          	if(d==="Republic of Korea")
          	{return "Korea";}
          	else if(d==="Russian Federation")
          	{ return "Russian"}
          	else if(d==="Saudi Arabia")
          	{ return "Saudi"}
          	else if(d==="United Kingdom")
          	{ return "UK"}
          	else if(d==="United States of America")
          	{ return "USA"}
          	else if(d==="South Africa")
          	{ return "Africa"}
          	else  {return d}
          	});
...
block_unit.append("i")
          .attr("class","c1 op2")
          .style("color",function(d){ return color(d,0.85)})
